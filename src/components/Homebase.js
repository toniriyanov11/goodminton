...
function Homebase(){
    ...
    return (
        <section id="homebase" className="p-4 py-12 text-center text-primary bg-white opacity-90">
            <p className="font-bold text-2xl flex items-center"><span class="h-1 bg-secondary inline-block mx-2 w-[30px]"></span>homebase.</p>
            ...
            <div className="mx-auto w-4/5 mt-12 mb-[10rem]">
                {
                    homebases.map((homebase)=>{
                        return(
                            <CardHomebase title={homebase.title} address={homebase.address}/>
                        )
                    }) 
                }
            </div>
